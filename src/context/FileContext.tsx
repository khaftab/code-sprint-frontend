...
import { findParentDirectory, getFileById, isFileExist } from "@/utils/file"
...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const createDirectory = useCallback(
        (
            ...
            const checkDirectoryExists = (
                ...
                if (directory.id === parentDirId) {
                    // Check if a directory with the same name exists in the current directory

                    return (
                        directory.children?.find(
                            (child) =>
                                child.type === "directory" &&
                                child.name === newDirectory.name,
                        ) || null
                    )
                } else if (directory.children) {
                    // Recursively check in child directories
                    for (const child of directory.children) {
                        if (child.type === "directory") {
                            const result = checkDirectoryExists(child)
            ...
            setFileStructure((prevFileStructure) => {
                const isExists = checkDirectoryExists(prevFileStructure)

                if (isExists) {
                    return prevFileStructure
                } else {
        ...
        (
            ...
            const checkFileExists = (
                ...
                if (directory.id === parentDirId) {
                    ...
                    for (const child of directory.children) {
                        if (child.type === "directory") {
                            const result = checkFileExists(child)
                            if (result) return result
                        }
                    }
                }
            ...
            const isExisits = checkFileExists(fileStructure)
    ...
    const renameFile = useCallback(
        (
            ...
            const renameInDirectory = (
                ...
                if (directory.type === "directory" && directory.children) {
                    // Find the target file and its parent directory
                    const targetFile = directory.children.find(
                        (item) => item.id === fileId,
                    )

                    // If target file is found in this directory, check for duplicates
                    if (targetFile) {
                        // Check if another file with the same name exists in this directory
                        const hasDuplicate = directory.children.some(
                            (item) =>
                                item.id !== fileId && item.name === newName,
                        )

                        if (hasDuplicate) {
                            isDuplicateFound = true
                            return directory // Don't rename if duplicate found
                        }
                    ...
                    // Target not found in this directory, check children recursively
                    return {
                        ...directory,
                        children: directory.children.map((item) => {
            ...
            // Try to rename in the file structure
            setFileStructure((prevFileStructure) => {
                const newStructure = renameInDirectory(prevFileStructure)
                return newStructure
            })

            // If duplicate was found, return false early
            if (isDuplicateFound) {
                return false
            }

            // If the file was successfully renamed, update other states
            if (isRenamed) {
                // Update both openFiles and activeFile in a single state transition
                setOpenFiles((prevOpenFiles) => {
                    const updatedOpenFiles = prevOpenFiles.map((file) =>
                        file.id === fileId ? { ...file, name: newName } : file,
                    )

                    // Update activeFile to reference the new object
                    setActiveFile((prevActive) =>
                        prevActive?.id === fileId
                            ? updatedOpenFiles.find((f) => f.id === fileId) ||
                              prevActive
                            : prevActive,
                    )
    ...
    const deleteFile = useCallback(
        (fileId: string, sendToSocket: boolean = true) => {
            ...
            // Remove the file from openFiles and update active file in one operation
            if (openFiles.some((file) => file.id === fileId)) {
                setOpenFiles((prevOpenFiles) => {
                    // First, filter out the deleted file
                    const updatedOpenFiles = prevOpenFiles.filter(
                        (file) => file.id !== fileId,
                    )

                    // Then, if the active file is being deleted, update it in the same operation
                    if (activeFile?.id === fileId) {
                        // Set to the first file in the updated list or null if empty
                        setActiveFile(
    ...
    const handleFileStructureSync = useCallback(
        ({
            ...
            console.log(
                "File structure sync received from server",
                fileStructure,
            )
    ...
    const handleFileCreated = useCallback(
        ({
            ...
            // check if file with same name and parentId already exists
